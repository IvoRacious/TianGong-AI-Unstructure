...
# 读取 error_file_id_2.csv 里的 id 到集合
error_file_ids = set()
with open("error_file_id_2.csv", "r") as ef:
    for line in ef:
        error_file_ids.add(line.strip())

for result in data:
    file_id = result[0]
    # 如果在 error_file_id_2.csv 里，跳过
    if file_id in error_file_ids:
        logging.info(f"Skipping file_id in error_file_id_2.csv: {file_id}")
        continue
    doi = result[1]
    ...
    if os.path.exists(pickle_path):
        ...
        if os.path.exists(file_path):
            try:
                ...
                try:
                    with open("error_file_id_2.csv", "a") as ef:
                        ef.write(f"{file_id}\n")
